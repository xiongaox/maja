              ...
              {dailyStats[selectedDay].records.map((tx) => (
                <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <div className="flex items-center gap-4">
                    <img
                      src={`https://api.dicebear.com/7.x/${tx.gender === 'girl' ? 'lorelei' : 'adventurer'}/svg?seed=${encodeURIComponent(tx.displayName || tx.name)}&backgroundColor=transparent&skinColor=ecbcb4,f2d3b1,f8e3c1`}
